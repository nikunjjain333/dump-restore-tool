...
def add_docker_compose_config(config: DockerComposeConfigCreate, db: Session = Depends(get_db)):
...
def delete_docker_compose_config_endpoint(config_id: int, db: Session = Depends(get_db)):
