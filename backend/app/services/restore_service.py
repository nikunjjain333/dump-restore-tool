...
def run_restore(db_type: str, params: Dict[str, Any], config_name: str, restore_password: Optional[str],
                ...
                restore_host: Optional[str] = None, restore_port: Optional[str] = None,
                stack_name: Optional[str] = None) -> Dict[str, Any]:
...
def _restore_postgres(params: Dict[str, Any], path: str, stack_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        def docker_exec(cmd):
            ...
            if not info.get('success'):
                return format_error_response(f"Could not find database container for stack '{stack_name}': {info.get('message')}")
            container_id = info['container']['id']
            docker_cmd = f"docker exec -e PGPASSWORD={password} {container_id} bash -c \"{cmd}\""
        ...
        try:
            ...
            result = docker_exec(drop_cmd) if stack_name else _run_host_command(drop_cmd, env=env)
        ...
        result = docker_exec(create_cmd) if stack_name else _run_host_command(create_cmd, env=env)
        if hasattr(result, 'returncode') and result.returncode != 0:
        ...
        result = docker_exec(restore_cmd) if stack_name else _run_host_command(restore_cmd, env=env)
        if hasattr(result, 'returncode') and result.returncode == 0:
