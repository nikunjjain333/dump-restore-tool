...
class DockerComposeConfigBase(BaseModel):
    name: str = Field(
        ..., min_length=3, max_length=100, description="Configuration name"
    )
    path: str = Field(..., description="Path to docker-compose.yml file")
    service_name: Optional[str] = Field(
        None, description="Optional service name for targeted operations"
    )
    flags: Optional[Dict[str, Any]] = Field(
        None, description="Additional Docker Compose flags"
    )
...
class DockerComposeOperationRequest(BaseModel):
    ...
    operation: str = Field(
        ..., description="Operation to perform (up, down, restart, logs, ps)"
    )
